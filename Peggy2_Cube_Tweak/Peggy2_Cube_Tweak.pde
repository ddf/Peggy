...
int cubeWidth = 15; // Cube size
float Angx = 0, AngxSpeed = 0.037; // rotation (angle+speed) around X-axis
float Angy = 0, AngySpeed = 0.052; // rotation (angle+speed) around Y-axis
float Ox=12.5,Oy=12.5; // position (x,y) of the frame center
int zCamera = 110; // distance from cube to the eye of the camera
float maxSeparation = 6.f; // how far apart the cubes can get.
float currSeparation = 0.f; // how separated they current are.
float sepSpeed = 0.02f; // how quickly they should animate in and out.
...
struct Cube
{
  ...
  void rotate(float angx, float angy)
  {
    ...
    {	
      ...
      boolean back = (pa->d > 110) && (pb->d > 110) && (pc->d > 110 );
      if (!back)
      {
        int j;
        for (j=0;j<4;j++)
  ...
  void draw( Peggy2 & frame )
  {
    ...
    {	
      e = edge+i;
      //if (!e->visible)
        frame.Line(screen[e->x].x,screen[e->x].y,screen[e->y].x,screen[e->y].y);  
    }
...
void loop()
{
  ...
  currSeparation += sepSpeed;
  if ( currSeparation >= TWO_PI )
  {
    currSeparation -= TWO_PI;
  }
  
  currRotation += rotSpeed;
  if ( currRotation >= TWO_PI )
  {
    currRotation -= TWO_PI;
  }
  
  const float maxZOffset = 75.f;
  
  float xoffset = sin( currSeparation ) * maxSeparation;
  float yoffset = cos( currRotation ) * maxSeparation;
  float zOffset = sin( (currSeparation + currRotation) ) * maxZOffset;
    
  // Clear frames
  frame1.Clear();
  frame2.Clear();
  
  const int kMaxRefresh = 10;
  const int kMinRefresh = 1;

  // Draw left cube
  Ox = 12.5 - xoffset;
  Oy = 12.5 - yoffset;
  zCamera = 110 + zOffset;
  int frame1Refresh = (int)map( zCamera, 110 - maxZOffset, 110 + maxZOffset, kMaxRefresh, kMinRefresh );
  ...
  Oy = 12.5 + yoffset;
  zCamera = 110 - zOffset;
  int frame2Refresh = (int)map( zCamera, 110 - maxZOffset, 110 + maxZOffset, kMaxRefresh, kMinRefresh );
  cube.rotate(Angx * 1.5f, Angy * 1.5f);
  cube.draw( frame2 );
  
  // Flicker !
  for (reps=0; reps< repNumber;reps++)
  {
   // frame1.RefreshAllFast(1); //Draw frame buffer 1 time
   // frame2.RefreshAllFast(10); //Draw frame buffer 10 times
    //frame1.RefreshAll(1); //
    //frame2.RefreshAllSlow(1); //
    if ( frame2Refresh >= frame1Refresh )
    {
      frame1.RefreshAll( frame1Refresh );
      frame2.RefreshAll( frame2Refresh );
    }
    else
    {
      frame2.RefreshAll( frame2Refresh );
